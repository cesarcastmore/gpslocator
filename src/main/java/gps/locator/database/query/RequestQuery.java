...
public class RequestQuery {
	...
	public static List<Request> findRequestsByBusiness(User business, String findby) {
		...
		db.add(Restrictions.eq("categoryname", address.getCategoryname()));
	...
	public static List<Request> getRequestsByBusiness(User business) {
		DAL db = new DAL();
		db.openSession();
		...
		db.createCriteria(Request.class);
		db.add(Restrictions.eq("categoryname", address.getCategoryname()));
		db.add(Restrictions.eq("user", business));
	...
	public static List<Request> getRequestsByClient(User user) {
		...
		db.add(Restrictions.isNotNull("categoryname"));
	...
	public static List<Request> getRequestsByClientAndService(User user, String name) {
		...
		db.createNamedQuery("Request.byCategoryName");
		db.setString(0, name);
	...
	public static List<Request> getNodes(Long requestId, User user) {
		...
		db.createSQLQuery(
				"SELECT * FROM Request r WHERE r.requestId IN (SELECT nodeId FROM Tree t WHERE t.parentId = ?  AND r.userId = ? )");
		db.setLong(0, requestId);
